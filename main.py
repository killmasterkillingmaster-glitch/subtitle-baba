...
def main():
    ...
    app.add_handler(MessageHandler(filters.Regex(r"(?i)^/send\s*more\s*channel$"), cmd_send_more))
    ...
    post_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(r"(?i)^/post$"), post_start)],
        states={
        ...
        fallbacks=[MessageHandler(filters.Regex(r"(?i)^/delete$"), cmd_delete)]
    )
    app.add_handler(post_conv)

    # Shortener Add
    short_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(r"(?i)^/add\s*shortn?er\s*account$"), add_shortener_start)],
        states={
    ...
    app.add_handler(MessageHandler(filters.Regex(r"(?i)^/remove\s*shortn?er\s*account$"), cmd_rem_short))

    # Premium Add
    prem_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex(r"(?i)^/add\s*premium$"), add_premium_start)],
        states={
            PREM_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_prem_id)],
            PREM_CONF: [MessageHandler(filters.Regex(r"(?i)^/hu\s*hu$"), confirm_prem)]
        },
        fallbacks=[MessageHandler(filters.Regex(r"(?i)^/delete$"), cmd_delete)]
    )
    app.add_handler(prem_conv)
    
    app.add_handler(MessageHandler(filters.Regex(r"(?i)^/remove\s*premium$"), cmd_rem_prem))
    app.add_handler(MessageHandler(filters.Regex(r"(?i)^/show\s*premium\s*list$"), show_prem))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, process_rem_prem), group=1)

    # Force Sub
    app.add_handler(MessageHandler(filters.Regex(r"(?i)^/force\s*sub$"), add_fsub))
    app.add_handler(MessageHandler(filters.FORWARDED & ~filters.COMMAND, receive_fsub_msg))

    print("🚀 Bot Started 100% Successfully!")
